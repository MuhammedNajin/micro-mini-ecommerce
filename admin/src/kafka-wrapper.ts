...
class KafkaWrapper {
  ...
  async connect(clientId: string, brokers: string[]) {
    ...
    this._producer = this._client.producer({ allowAutoTopicCreation: true });
    this._groupId = 'admin-service-group';
    this._consumer = this.client.consumer({ groupId: "admin-service-group"})
